...
# args = list()
# args[["glob"]] = "./outputs/bin_stats/*tsv"
# args[["output"]] = "./outputs/bedpe_bin_stats_GSM5067062_2020_6544_human_bedpe.1000lines_test.feather"
# args[["--in-tsv"]] = TRUE
...
output <- ddply(data.frame(paths), 'paths', function(z) {
      ...
      if (args[['--in-tsv']]) {
          ...
          infile=suppressMessages(read.table(path, header = T, sep = "\t"))
      } else if (args[['--in-csv']]) {
